document.addEventListener("DOMContentLoaded", () => {
  ...
  async function completeSignup(token) {
    ...
    // Sign up user (Auth v3)
    const { data, error } = await client.auth.signUp({
      email,
      password,
      options: {
        emailRedirectTo: "https://www.spinwintoken.com/registerconfirm.html?type=signup&ref=" + ref,
        captchaToken: token
      }
    });
    ...
    // If user object is missing, treat as failure
    if (!data || !data.user) {
    ...
    // Direct INSERT into UsersSQL (no pre-SELECT)
